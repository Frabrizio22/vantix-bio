// ============================================
// VANTIX BIO - COMPLETE APPS SCRIPT (CLEANED)
// Orders + Inventory + Dashboard + Payment Confirmations
...
function handleOrder(data) {
  ...
  if (paymentMethod.toLowerCase().indexOf('zelle') !== -1) {
    sendConfirmationEmail(data, orderNumber);
  }
  // Credit card confirmations sent via payment_confirmed webhook
...
function handlePaymentConfirmation(data) {
  ...
  for (var i = 1; i < ordersData.length; i++) {
    if (String(ordersData[i][0]).trim() === orderNumber) {
      rowIndex = i + 1;
      break;
    }
  }
  ...
  var orderRow = ordersData[rowIndex - 1];
  var orderData = {
    customer_email: orderRow[2],
    customer_name: orderRow[3],
    items: parseItems(orderRow[9]),
    total: orderRow[16],
...
function parseItems(productText) {
  ...
  parts.forEach(function(part) {
    ...
    if (match) {
      items.push({ name: match[1], quantity: parseInt(match[2]) });
    }
  });
...
function sendConfirmationEmail(data, orderNumber) {
  try {
    var items = data.items || [];
    var paymentMethod = (data.payment_method || '').toLowerCase();
    var isZelle = paymentMethod.indexOf('zelle') !== -1;
    var firstName = (data.customer_name || '').split(' ')[0] || 'there';
    var total = parseFloat(data.total || 0).toFixed(2);
    var subtotal = parseFloat(data.subtotal || data.total || 0).toFixed(2);
    var discount = parseFloat(data.discount || 0).toFixed(2);
    var shipping = parseFloat(data.shipping || 0).toFixed(2);
    
    // Build item rows with clean formatting
    var itemRows = '';
    items.forEach(function(item) {
      ...
      itemRows += '<tr style="border-bottom:1px solid #f0f0f0">' +
        ...
        '<div style="font-size:14px;font-weight:500;color:#0F1B2D;margin-bottom:2px">' + item.name + '</div>' +
        '<div style="font-size:13px;color:#9CA3AF">Qty ' + qty + ' × $' + price.toFixed(2) + '</div>' +
        '</td>' +
        '<td style="text-align:right;padding:12px 0;vertical-align:top">' +
        '<div style="font-size:14px;font-weight:600;color:#0F1B2D">$' + lineTotal + '</div>' +
        '</td>' +
        '</tr>';
    });
    
    var subject = 'Order Confirmed #' + orderNumber + ' - Vantix Bio';
    ...
    var htmlBody = '<!DOCTYPE html>' +
      ...
      '<div style="font-size:18px;font-weight:600;color:#0F1B2D">#' + orderNumber + '</div>' +
      '</div>' +
      ...
      (isZelle ? 
        ...
        'Include <strong>#' + orderNumber + '</strong> in note' +
        '</div></div>' :
