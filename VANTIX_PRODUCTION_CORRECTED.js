// ============================================
// VANTIX BIO - PRODUCTION APPS SCRIPT (CORRECTED)
// Handles: Orders (doPost) + Dashboard Data (doGet) + Bankful Callbacks
// Last Updated: June 25, 2026
// Column Structure: A-Y with Ship Cost Actual in T, Net Profit in U
// 
// FIXES APPLIED:
// - payment_confirmed handler for Bankful callbacks (was missing in some versions)
// - Auth bypass for waitlist/email_capture (Worker sends no api_key for these)
// - parseItems now includes price lookup for confirmation emails
// - Status dropdown includes "Completed"
// - Confirmation email sent for ALL orders (CC customers also get immediate email)
// ============================================
...
// Price lookup for confirmation emails (used when re-reading orders from sheet)
const PRODUCT_PRICES = {
  'Tirzepatide 30mg': 64.99, 'Tirz 30mg': 64.99,
  'Retatrutide 20mg': 59.99, 'Reta 20mg': 59.99,
  'Semaglutide 10mg': 54.99, 'Sema 10mg': 54.99,
  'BPC-157 10mg': 44.99, 'BPC 10mg': 44.99,
  'TB-500 10mg': 49.99, 'TB 10mg': 49.99,
  'GHK-Cu 100mg': 39.99, 'GHK 100mg': 39.99,
  'CJC-1295 10mg': 44.99, 'CJC 10mg': 44.99,
  'Ipamorelin 5mg': 39.99, 'Ipa 5mg': 39.99,
  'MOTS-c 10mg': 49.99, 'MOTS-C 10mg': 49.99, 'MOTS 10mg': 49.99,
  'NAD+ 1000mg': 79.99, 'NAD+ 1g': 79.99,
  'Tesamorelin 10mg': 54.99, 'Tesa 10mg': 54.99,
  'AOD-9604 10mg': 44.99, 'AOD 10mg': 44.99
};
...
// ============================================
function doGet(e) {
  var inputPassword = (e && e.parameter && e.parameter.password) || '';
  ...
  var ss = SpreadsheetApp.openById('1n1YnFDUHyufWPXLAFOxz6RNkPGLr6gajG8JVc00cxTs');
  var ordersSheet = ss.getSheetByName('Orders');
  var filterMonth = (e && e.parameter && e.parameter.month) || null;
  
  var ordersData = ordersSheet.getDataRange().getValues();
  var orders = ordersData.slice(1).filter(function(row) { return row[0]; });
  
  // COLUMN INDICES (A-Y)
  var COL_ORDER_NUM = 0;   // A
  var COL_DATE = 1;        // B
  var COL_EMAIL = 2;       // C
  var COL_NAME = 3;        // D
  var COL_PRODUCT = 9;     // J
  var COL_QTY = 10;        // K
  var COL_PAYMENT = 11;    // L
  var COL_SUBTOTAL = 12;   // M
  var COL_DISCOUNT_CODE = 13; // N
  var COL_DISCOUNT = 14;   // O
  var COL_SHIPPING = 15;   // P
  var COL_TOTAL = 16;      // Q
  var COL_COGS = 17;       // R
  var COL_CC_FEES = 18;    // S
  var COL_NET_PROFIT = 20; // U
  var COL_STATUS = 21;     // V
...
function doPost(e) {
  try {
    ...
    // Only require API key for orders and payment confirmations
    // (Worker sends waitlist/email_capture without api_key)
    if (action === 'order' || action === 'payment_confirmed') {
...
// ============================================
// PAYMENT CONFIRMATION (Bankful Callback)
// Called by Worker when credit card payment succeeds
// ============================================
function handlePaymentConfirmation(data) {
  ...
  if (!orderNumber) {
    throw new Error('Order number required for payment confirmation');
  }
  ...
  for (var i = 1; i < ordersData.length; i++) {
    if (String(ordersData[i][0]).trim() === String(orderNumber).trim()) {
  ...
  if (rowIndex === -1) {
    Logger.log('Order not found for confirmation: ' + orderNumber);
    return ContentService.createTextOutput(JSON.stringify({
      status: 'error',
  ...
  // Update status to Paid (column V)
  ordersSheet.getRange('V' + rowIndex).setValue('Paid');
  
  // Build order data for confirmation email
  // Parse items from product text in column J and look up prices
  var productText = String(ordersData[rowIndex - 1][9]);
  var parsedItems = parseItemsWithPrices(productText);
  var orderTotal = ordersData[rowIndex - 1][16]; // Column Q
  
  var orderData = {
    customer_email: ordersData[rowIndex - 1][2],  // Column C
    customer_name: ordersData[rowIndex - 1][3],    // Column D
    items: parsedItems,
    total: orderTotal,
    payment_method: 'Credit Card'
  };
  
  sendConfirmationEmail(orderData, orderNumber);
  
  Logger.log('Payment confirmed for order: ' + orderNumber);
...
// Parse "Product (x2), Product2 (x1)" into items array WITH price lookup
function parseItemsWithPrices(productText) {
  if (!productText) return [];
  
  var items = [];
  var parts = String(productText).split(', ');
...
// ORDER HANDLER
// Sheet Columns (A-Y):
// A=Order#, B=Timestamp, C=Email, D=Name, E=Phone, F=Address, G=City,
// H=State, I=Zip, J=Product, K=Qty, L=Payment, M=Subtotal,
// N=Discount Code, O=Discount, P=Shipping, Q=Total, R=COGS,
// S=CC Fees, T=Ship Cost Actual, U=Net Profit, V=Status,
...
function handleOrder(data) {
  ...
  ordersSheet.appendRow([
    ...
    data.state || '',               // H: State
    data.zip || '',                 // I: Zip
    productText,                    // J: Product
    totalQty,                       // K: Qty
    paymentMethod,                  // L: Payment
    subtotal.toFixed(2),            // M: Subtotal
    data.discount_code || '',       // N: Discount Code
    discount.toFixed(2),            // O: Discount
    shipping.toFixed(2),            // P: Shipping
    total.toFixed(2),               // Q: Total
    cogs.toFixed(2),                // R: COGS
    ccFees.toFixed(2),              // S: CC Fees (4.4%)
    '',                             // T: Ship Cost Actual (manual entry)
    '',                             // U: Net Profit (will be formula)
    'Pending',                      // V: Status
    '',                             // W: Shipped
    '',                             // X: Tracking
    ''                              // Y: Batch #
  ...
  // Add dropdown validation to Status column (V)
  var statusRule = SpreadsheetApp.newDataValidation()
    .requireValueInList(['Pending', 'Paid', 'Shipped', 'Delivered', 'Completed'], true)
    .setAllowInvalid(false)
    .build();
  ordersSheet.getRange('V' + lastRow).setDataValidation(statusRule);
  
  updateInventory(items);
  sendOrderNotification(orderNumber, data);
  
  // Send confirmation email for ALL orders (both Zelle and CC)
  // CC customers get a second "payment confirmed" email when Bankful callback comes
...
function sendConfirmationEmail(data, orderNumber) {
  try {
    ...
    var itemRows = items.map(function(item) {
      var price = parseFloat(item.price) || lookupPrice(item.name);
      var lineTotal = (price * (parseInt(item.quantity) || 1)).toFixed(2);
      return '<tr><td>' + (item.quantity || 1) + 'x ' + item.name + '</td><td>$' + lineTotal + '</td></tr>';
    }).join('');
