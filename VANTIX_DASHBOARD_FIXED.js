...
function handlePaymentConfirmation(data) {
  ...
  var orderData = {
    customer_email: ordersData[rowIndex - 1][2],
    customer_name: ordersData[rowIndex - 1][3],
    items: parseItems(ordersData[rowIndex - 1][9]),
    total: ordersData[rowIndex - 1][16],
...
function parseItems(productText) {
  // Convert "Product (x2), Product2 (x1)" to items array
...
function handleOrder(data) {
  ...
  var statusRule = SpreadsheetApp.newDataValidation()
    .requireValueInList(['Pending', 'Paid', 'Shipped', 'Delivered'], true)
  ...
  // Only send confirmation email for Zelle (immediate) or if payment already confirmed
  if (paymentMethod.toLowerCase().indexOf('zelle') !== -1) {
    sendConfirmationEmail(data, orderNumber);
  }
  // Credit card confirmations will be sent via payment confirmation webhook
